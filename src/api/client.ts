 ...
 * - 응답 인터셉터: 서버 에러 코드 정규화 후 throw
 *
 * ⚠️ 알림(토스트)은 이 파일에서 직접 표시하지 않습니다.
 *   antd message/notification 정적 함수는 React 컨텍스트 밖(axios 인터셉터)에서
 *   호출하면 "Static function can not consume context" 경고가 발생합니다.
 *   에러 알림은 각 호출부(페이지/스토어)에서 uiStore.notifyError()로 처리합니다.
 */
...
client.interceptors.response.use(
  ...
  // 2xx 외: 에러 정규화 후 throw
  // 알림 표시는 하지 않음 — 호출부에서 notifyError()로 처리
